...
| **An address Growther issues**, like `<label>.rc.growther.ai` | Through a Cloudflare Tunnel that Growther creates for your install | Cloudflare                     |

The first two need nothing from Growther and keep working without it. The third is the one
...
| The address Growther issues                        | **Cloudflare**                 | Nobody, for ordinary requests. Streamed content is not signed |
| Signing in at `rc.growther.ai`                     | Growther sees the sign-in only | Nobody                                                        |
...
Your session content never passes through Growther's servers. While Remote Control is on,
